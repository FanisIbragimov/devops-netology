...
3. К имени файла добавляется пометка (deleted). Если его нужно восстановать, то можно послать процесс с сигналом 3 по пути с pid-ом процесса. он восстановится. Смею предположить, что это можно сделать так ls -la > 1.txt 2> /dev/null. 
